    ...
    def _generate_custom_terrain(self):
        ...
        for segment in self.terrain_config:
            ...
            if segment_type == "flat":
                ...
                angle = segment.get("angle", 15)  # 默认15度
                ...
                if height_change is None:
                    ...
                    height_change = -abs(height_change)  # 确保是负值
...
def create_custom_terrain_config(segments):
    ...
    Args:
        segments: 地形段列表，每个段是一个字典，包含:
                 - type: 'flat', 'up', 'down', 'stairs', 'pit'
                 - length: 段长度
                 - 其他参数根据类型而定

    示例:
        # 创建复杂地形：平坦->上升->平坦->下降->坑->楼梯
        config = create_custom_terrain_config([
            {'type': 'flat', 'length': 40},
            {'type': 'up', 'length': 20, 'angle': 25},
            {'type': 'flat', 'length': 30},
            {'type': 'down', 'length': 15, 'angle': 20},
            {'type': 'pit', 'length': 10, 'depth': 2.0},
            {'type': 'stairs', 'length': 24, 'direction': 'up', 'width': 4, 'height': 0.5}
        ])
    """
    return segments
